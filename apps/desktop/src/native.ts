...
export interface ProductionSettings { outputPath: string; width: number; height: number; fps: number; supersampling: number; motionBlurSamples: number; substeps: number; codec: "h264" | "hevc" | "prores422hq" | "prores4444"; }
...
export interface LiveInputValue { source: string; value: number; }
...
export const randomizeProject = (projectPath: string, project: ProjectData, amount: number, variation: number) => invoke<ProjectData>("randomize_project", { projectPath, project, amount, variation });
export const morphProject = (project: ProjectData, left: string, right: string, amount: number) => invoke<ProjectData>("morph_project", { project, left, right, amount });
export const makeSeamlessLoop = (project: ProjectData) => invoke<ProjectData>("make_seamless_loop", { project });
export const saveProject = (path: string, project: ProjectData) => invoke<string>("save_project", { path, project });
...
export const cancelProductionRender = () => invoke<void>("cancel_production_render");
export const openProductionOutput = () => invoke<void>("open_production_output");
export const openProductionFolder = () => invoke<void>("open_production_folder");
export const queryMidiPorts = () => invoke<string[]>("midi_ports");
export const connectMidi = (index: number) => invoke<void>("midi_connect", { index });
export const listenOsc = (address: string) => invoke<void>("osc_listen", { address });
export const queryLiveInputValues = () => invoke<LiveInputValue[]>("live_input_values");
