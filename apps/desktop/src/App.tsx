...
import { loadPreviewAudio, loadProject, queryGpuInfo, queryPreviewStats, resetPreview, resizeViewport, saveProject, seekPreview, setPreviewPlaying, setPreviewQuality, updateProject, type GpuSummary, type PreviewStats, type TimelineAudio } from "./native";
...
export function App() {
  ...
  return <main className="studio-shell"><header className="topbar"><div><span className="brand">RUSTIQUE</span><span className="subtitle">particle studio {dirty ? "• unsaved" : ""}</span></div><button onClick={() => void run(async () => setGpu(await queryGpuInfo()))}>Inspect GPU</button></header>
    ...
    <section className="panel timeline"><h2>Audio & timeline</h2><Timeline project={project ?? null} audio={timelineAudio} cursorSeconds={project ? (stats?.frameIndex ?? 0) / project.fps : 0} onSeek={(seconds) => project && void seekPreview(Math.round(seconds * project.fps))} onProjectChange={edit} /></section><aside className="panel render"><h2>Render</h2><label>Output</label><select defaultValue="preview"><option value="preview">Preview · 1080p</option><option value="final">Final · 4K</option></select><button disabled>Queue render</button></aside>
  </main>;
}
